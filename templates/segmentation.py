...
from thtrainer import tensorboard, callbacks
...
def make_transforms(train=True):
    '''You can refer to `https://github.com/jcoral/Transforms/blob/master/detection.py`'''
    transforms = [
        # TODO: Resize
    ]
    if train:
    ...
    def _wrapper(X, y):
        ...
        def closure():
            ...
            if isinstance(output, (tuple, list)):
                output, auxs = output[0], output[1:]
            pair = (output, y)
            trainer.metrics.update(pair)
            loss = trainer.loss_fn(*pair)
            if auxs is not None:
                loss = loss + sum([trainer.loss_fn(aux, y) for aux in auxs])
            loss.backward()
            return loss
